...
Hot-reload should pick up the resolved source automatically. If the daemon stack is wedged after you finish resolving, recover from inside the worktree with:

```
docker compose up --build --wait
```

That rebuilds against the now-clean source. **Do not reset the worktree or use any "hard" reset command** — those wipe in-progress work.
