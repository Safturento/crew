...
describe('installNodeModules', () => {
  ...
  beforeEach(() => {
    tmp = join(tmpdir(), `crew-npm-test-${process.pid}-${Date.now()}`);
  ...
  it('spawns `npm install` with the given cwd and env', async () => {
  ...
  it('returns rc 0 and the log path on success', async () => {
    execaMock.mockReturnValue(fakeProcess(0) as unknown as ReturnType<typeof execa>);
    const result = await installNodeModules({
      worktree: tmp,
      key: 'KAN-99',
      env: process.env,
    });
    expect(result.rc).toBe(0);
    expect(result.logPath).toBe('/tmp/crew-npm-install-KAN-99.log');
  });
