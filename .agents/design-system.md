...
| `RunDrawer`          | `881:1216`                      | `packages/dashboard/src/components/runner/RunDrawer.tsx`         |

The runner composites (`SupervisorCard`, `ProcessRow`, `FailedStartCard`, and the interim `ViewOutputModal`) landed via CREW-245 (Epic CREW-235, Runner page). `SupervisorCard` / `ProcessRow` resolve to component sets (a `state` variant axis). `ProcessRow` / `FailedStartCard` and the runner's other rows (Unmanaged, Queued, Recently-ended) all render through the shared `Row` primitive (below). Verification record: `docs/visual-fidelity-reports/CREW-245.md`.

**CREW-291 — the run drawer absorbs `ViewOutputModal`.** `RunDrawer` (`packages/dashboard/src/components/runner/RunDrawer.tsx`, Figma `RunDrawerBody` set `881:1216`, Epic CREW-249 / plan T3) is the per-run drawer opened by clicking any run row (live `ProcessRow` / `FailedStartCard` / `RecentlyEnded`). It reuses the `Drawer` shell and renders a header (key + `CommandBadge` + state `Badge`, plus a `pr_open` PR `Badge` for finished runs), a meta line (pid/pgid for live runs · project · relative timestamp), a failed-start **Diagnosis** (the structured check / headline / amber remediation), and a **Console output** region fed by `GET /api/runs/:key/startup-log` (`useStartupLog`, poll-tailed while in-flight). The Figma set's `state` axis (failed-start / running / ended) maps to the `RunDrawerSource` discriminated union. This **replaced** `ViewOutputModal` (the interim Diagnosis + Output modal mapped to `ViewOutputContent` `774:1150`); both `ViewOutputModal.tsx` and its `.figma.tsx` were deleted, and the row `Inspect` / View-output affordance now opens the drawer. The matching `SupervisorDrawerBody` (`882:1216`) is built in Figma but its code counterpart (`SupervisorDrawer`) lands with plan T4, not CREW-291.
...
`SupervisorDrawer` (`packages/dashboard/src/components/runner/SupervisorDrawer.tsx`, CREW-292) is the Runner page's supervisor detail drawer, opened by clicking the `SupervisorCard` (which now passes `onOpen` → the `Row` `onActivate`, with the lifecycle buttons `stopPropagation`-guarded). It composes the `Drawer` shell with a `crew / runner` breadcrumb header (Supervisor title + running/down `Badge` + heartbeat/last-seen meta) and a console region that poll-tails the supervisor's process-management log via `useSupervisorLog` → `GET /api/runner/supervisor-log`. Like `FinishSteps` it's figma-less feature-internal in code — no `.figma.tsx` — even though the Figma has a `SupervisorDrawerBody` counterpart (`882:1216`, Composites): the project ships no Code Connect, so the drawer is verified against that node by `visual-fidelity-check`, not a mapping file. The Figma meta line also depicts workers/uptime/pid, but `SupervisorView` carries only `online`/`lastSeen` today (same wire limitation `SupervisorCard` documents), so those fill in once the heartbeat payload grows. Verification record: `docs/visual-fidelity-reports/CREW-292.md`.
