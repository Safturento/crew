...
- CREW-244's visible failed-start surface (`reportLaunching` → `launching` runs row → `reportFailedStart` → `failedToStart` list) is wired through `runTrackedPreflight`, which only wraps `prepareAgentEnvironment` (`run.ts:432`) — i.e. it runs *after* the early gate. Despite the comment "register the run as `launching` BEFORE preflight," the worktree-available check at `run.ts:261` `process.exit`s before `reportLaunching` is ever called, so no `launching` row is minted. And even inside the wrapper, `reportFailedStart` only fires for `err instanceof PreflightError` (`preflight-tracking.ts:60`); `requireWorktreeAvailable` throws a plain `Error`.

Net: the entire early-gate failure class (missing tools, gh-auth, **worktree-already-exists**) never reaches the daemon's visible run/agent state. The runner's heartbeat reaper (CREW-288 `reapDead`) then drops the dead detached child ~2s later and logs a bare `reaped 1 dead process(es): <key>` — the only trace an operator sees, with no reason attached.

**Why noticed:** 2026-06-30 investigation of `crew run HAI-12` (home-assistant project) "immediately getting reaped" with nothing in the supervisor logs. Root-caused from `~/.crew/startup/HAI-12.jsonl` (`crew_startup_preflight` `failed`: "worktree already exists at /home/safturento/Repos/home-assistant-HAI-12") cross-read against `~/.crew/runner/runner.log` (`claimed action 138 → launched → reaped 1 dead process(es): HAI-12`, ~2s later). The stale worktree was left by a June-16 run that created it (`crew_startup_worktree` completed) and was interrupted without teardown — branch `HAI-12` sits at `origin/main` (`8771f45`), zero commits, nothing to recover. User's reaction: "wasn't CREW-287 supposed to fix this?" — no; CREW-287 reclaims an orphan *branch* inside the `crew_startup_worktree` phase (`reconcileOrphanBranch`, `run.ts:304`), which never runs because the earlier preflight gate aborts first. Different failure mode, earlier phase.

**Anchors:**

- `packages/cli/src/commands/run.ts` — early preflight gate `:236-278` (`preflightTools`, `requireGithubAuth`, `requireWorktreeAvailable`); `failStartupPhase` `:856-878` (emits JSONL + `fail()`/exit); `runTrackedPreflight` call site `:432` (wraps only `prepareAgentEnvironment`, *after* the gate).
- `packages/cli/src/lib/run/preconditions.ts:7` — `requireWorktreeAvailable` throws a plain `Error` (the "worktree already exists" message), not a `PreflightError`.
- `packages/cli/src/lib/run/preflight-tracking.ts` — `runTrackedPreflight`; `reportFailedStart` gated on `instanceof PreflightError` (`:60`).
- `packages/daemon/src/services/IngestService.ts` — `watchStartupEvents` → `ingestStartupEvent` (`:356`) → `recordError` (`:397`, writes a `state_transitions` row keyed on `agentKey` with **no** `agents` row); `getCachedAgentState` (`:746`) returns `init` when no row exists.
- `packages/daemon/src/routes/runs.ts` — `POST /api/agents/runs` register (creates the `agents` row) runs *after* claude spawn.
- Sibling followups (same root, different slice): [[#2026-06-17--only-preflighterror-becomes-a-structured-failed-start-dockernpmplaywright-init-failures-dont]] (docker/npm inside `prepareAgentEnvironment` also miss the failed-start surface — but they *do* get a `launching` row; the early-gate case doesn't even get that) and [[#2026-06-17--failed-start-rows-render-as-plain-error-agents-in-the-main-grid]] (how a failed-start *should* render once it exists). Also related: the CREW-287/288 resolution notes both explicitly punted "a run that dies before registering still has no `runs` row for the daemon to settle" to CREW-249 — CREW-249 shipped the drawers + failed-start plumbing but never rewired the early gate to feed it.

**What's been considered:** Two design directions raised in the surfacing conversation (both feed the follow-on brainstorm, not yet decided):

1. **Create the agent row as the very first thing a `crew run` does**, and have it track *all* state from the moment the run is initiated — so preflight/worktree/env failures land on a real, visible entity instead of an orphan transition. This subsumes the narrow "route `failStartupPhase` through `reportFailedStart`" fix by making "no agent row yet" structurally impossible.
2. **Retire the Runner page as a distinct screen.** The agent rows there are shallow clones of the Agents page; move the supervisor action buttons into a drawer toggled from the **runner pill in the header**, and fold the failed/queued/recently-ended surfaces into the main grid (which, per (1), would now include pre-register failures).

Narrow-fix alternative if the above is too big: move `reportLaunching` genuinely ahead of the early gate and make `failStartupPhase` emit a `reportFailedStart` (or convert the gate checks to throw `PreflightError`). Plus a self-healing angle — extend CREW-287's "safe to auto-reclaim" logic from orphan *branches* to orphan *worktree directories* (a leftover worktree at `origin/<default>` with no unique commits is safe to delete + recreate), run in preflight, so the common wedge self-heals instead of needing `crew restart --hard`.

**Shape of work:** Likely an Epic — spans CLI dispatch (`run.ts` lifecycle reorder + agent-row-first), daemon (agent/run row created at launch, failed-start feeder from the early gate), and dashboard (runner-page retirement, header-pill drawer, grid folding). The brainstorm should decide scope boundaries and whether this reopens/extends CREW-249. No open ticket tracks the gap today (verified: 0 open CREW tickets matching preflight/worktree/failed-start/restart).

**Open questions:** Does creating the agent row at launch-time change the semantics of the Agents grid (a run that fails preflight becomes a permanent grid entry — same concern as the CREW-244 "failed-start renders as error" followup)? If the Runner page is retired, where do supervisor-level controls that aren't per-agent (start/stop the whole runner, view management log) live — the header-pill drawer only? Is there still value in a separate "queued but not yet launched" surface once agent rows exist from initiation? Should safe worktree-dir auto-reclaim be default-on (self-heal) or require an explicit `--reclaim`/restart action?
