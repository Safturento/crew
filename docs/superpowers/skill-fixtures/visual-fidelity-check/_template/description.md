...
1. **<component name> — <property/affordance>.** Be specific: which variant, which property, what's expected vs what's rendered. Include the file path and line range if relevant. Example: *"`<Button color='running' intensity='mid'>` — outline missing. Figma binds `border-slate-500` (1px), code's pillSurfaceClasses emits the class correctly but the rendered button shows no border. Possible class-precedence issue at `ui/button.tsx:base`."*
