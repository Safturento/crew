...
> **Implementation constraint — cache callback-refs per index.** The hook MUST return the same `(el) => void` callback for a given index across renders (cache in a `useRef<Map<number, fn>>`). Naively writing `useCallback((idx) => (el) => {...}, [])` returns a fresh inner arrow on every `refFor(i)` call — React's callback-ref semantics then detach + reattach every section every render. `ResizeObserver.observe()` fires its initial-size callback on every observe(), which schedules a state update that re-renders, which produces fresh refs, which re-observes — infinite loop. As a second line of defence, the height-update `setHeights` updater MUST return the SAME array reference when every new height equals the prior value (otherwise a future caller with unstable refs would still spin). See `packages/dashboard/src/components/Timeline/useSectionHeights.ts` for the canonical implementation. PR #275's first iteration shipped without these guards and produced an unreliable section-collapse regression in the browser; jsdom's no-op `ResizeObserver` stub hid the loop from the test suite.
...
- **ResizeObserver thrash.** If the section content changes frequently (live mode adding events), the observer fires per-resize. Mitigate by throttling height updates via `requestAnimationFrame` in the resize callback. Pair with the cached-ref + identity-preserving `setHeights` constraints above — a single missing guard turns this from "small thrash" into a render loop that the no-op jsdom `ResizeObserver` stub will silently hide.
