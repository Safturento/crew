...
Grid layout (desktop): `100px 90px 1fr 90px 70px auto` columns, 16px gap, 12px·16px padding.

Columns left → right:
1. **State pill** — `<StateBadge state={a.state} intensity={tweaks.stateIntensity} />`
2. **Key** — e.g. `KAN-31` (mono, 12px, var(--text-2)).
3. **Title** — single-line ellipsized ticket title (13.5px, var(--text)).
4. **Runtime** — e.g. `33m 04s` (mono, tabular-nums, right-aligned). Add `.runtime-live` modifier for active states (`running`, `initializing`) — adds a pulsing 5px blue dot via `::after` (animation `pulse 1.6s infinite`).
5. **Tokens** — formatted count, e.g. `48.2k` (mono, tabular-nums, right-aligned).
6. **Quick action** — context-sensitive button per state:
   - `waiting` → `Answer` (accent button)
   - `pr_open` → `View PR ↗` (ghost button)
   - `error` → `Retry` (ghost)
   - `running` / `initializing` / `idle` → no button (column reserves space, stays empty for alignment)
   - `finished` → `Archive` (ghost)
   Stop propagation on click — quick actions don't open the drawer.
...
3. **Timeline** — the transcript. Components: `Timeline` → `StateSegmentGroup` → `ToolCallCard`.

   - **Controls bar**: search input (filters tool calls by summary text) + Live toggle (pill switch — when ON, a 1.6s tick simulates new events appending at the bottom; LED-style blue dot when live).
